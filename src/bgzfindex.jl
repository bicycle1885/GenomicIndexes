...
# Virtual offset
const VOffset = UInt64

# Data chunk
const Chunk = Tuple{VOffset,VOffset}

# Binning index
const BinIndex = Dict{UInt32,Vector{Chunk}}

# Linear index
const LinearIndex = Vector{VOffset}
...
function overlapchunks(index::BGZFIndex, seqid::Integer, interval::UnitRange{<:Integer})
...
function reduce!(chunks)
    ...
    while i < endof(chunks)
        chunk = chunks[i]
        next = chunks[i+1]
        if chunk[2] > next[1] || next[1] - chunk[2] ≤ merge_threshold
            # merge overlapping or close chunks
            chunks[i] = (chunk[1], max(chunk[2], next[2]))
            deleteat!(chunks, i + 1)
            continue
        end
        i += 1

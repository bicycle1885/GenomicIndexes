...
"""
    Tabix(input::AbstractString)

Load a Tabix index from `input`.
"""
function Tabix(filename::AbstractString)
    return open(x->Tabix(x), filename)
end
...
"""
    seqnames(tabix::Tabix)

Return the list of sequence names.
"""
function seqnames(tabix::Tabix)
    return copy(tabix.names)
end

function findtabix(filepath::AbstractString)
    tbipath = string(filepath, ".tbi")
    return isfile(tbipath) ? Tabix(tbipath) : nothing
end

function overlapchunks(tabix::Tabix, seqname::AbstractString, interval::UnitRange{<:Integer})
    seqid = findfirst(tabix.names, seqname)
    if seqid == 0
        throw(ArgumentError("failed to find sequence name '$(seqname)'"))
    end
    return overlapchunks(tabix.index, seqid, first(interval):last(interval))
end
